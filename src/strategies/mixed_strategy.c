...
int x13_process_syn(struct mypacket *packet)
{
    ...
    send_fake_SYN(sip, sport, dip, dport, packet->ip4.tcphdr->th_seq); 
    send_fake_SYN(sip, sport, dip, dport, packet->ip4.tcphdr->th_seq); 
    send_fake_SYN(sip, sport, dip, dport, packet->ip4.tcphdr->th_seq); 
...
int x13_process_synack(struct mypacket *packet)
{
    ...
    send_RST_super(dip, dport, sip, sport, packet->ip4.tcphdr->th_ack);
    send_RST_super(dip, dport, sip, sport, packet->ip4.tcphdr->th_ack);
    int seq_num = htonl(ntohl(packet->ip4.tcphdr->th_ack) + (rand() % 10000000 + 1000000));
    send_fake_payload(dip, dport, sip, sport, seq_num, htonl(ntohl(packet->ip4.tcphdr->th_seq)+1)); 
    send_fake_payload(dip, dport, sip, sport, seq_num, htonl(ntohl(packet->ip4.tcphdr->th_seq)+1)); 
    send_fake_payload(dip, dport, sip, sport, seq_num, htonl(ntohl(packet->ip4.tcphdr->th_seq)+1)); 
